...
class Silhouette(base_internal_clustering.InternalMetric):
    """
    ...
    def update(self, method, point, label, sample_weight=1.0):
        distance_closest_centroid = math.sqrt(
            utils.math.minkowski_distance(method.centers[label], point, 2)
        )
    ...
    def revert(self, method, point, label, sample_weight=1.0, correction=None):
