...
class DenStream(base.Clusterer):
    ...
    The training of a new point `p` is performed based on the following algorithm, which

    * Try to merge `p` into either the nearest `p-micro-cluster`, `o-micro-cluster`, or create a new `o-micro-cluster`
    and insert it into the outlier buffer.

    * For each `T_p` iterations, consider the weights of all `p-` and `o-` micro clusters. If their weights are smaller
    than a certain threshold (two different thresholds for two types of micro clusters), the micro cluster will be deleted.

    * When a generating cluster resquest arrives, a variant of the DBSCAN algorithm [^2] from Ester et al. will be applied.
    Finally, the label of the new point will be determined by finding the nearest cluster with respect to that point.
    ...
    def learn_one(self, x, sample_weight=None):
        ...
        Training a new point includes two following main tasks. First, the two weight thresholds
        for `p-` and `o-` micro clusters will calculated. For each micro cluster, if its weight
        is smaller than its respective threshold, that  micro cluster will be deleted.
    ...
    def predict_one(self, x, sample_weight=None):
        ...
        When a clustering request arrives, a variant of DBSCAN algorithm [^2] is applied on the set of on-line
