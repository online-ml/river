...
class RobustSoftLearningVectorQuantization(base.MiniBatchClassifier):
    ...
    >>> from river.prototype import RobustSoftLearningVectorQuantization
    >>> import pandas as pd
    >>>
    >>> # Setup a data stream
    ...
    def _validate_train_params(self, train_set, train_lab, classes=None):
    ...
    def learn_many(self, X: pd.DataFrame, y: pd.DataFrame, classes=None):
        ...
        if set(unique_labels(y)).issubset(set(self.classes_)) or \
            ...
            X, y = self._validate_train_params(X, y, classes=classes)
        else:
    ...
    def learn_one(self, x: dict, y: base.typing.Target, classes=None) -> "Classifier":
        """Fit the LVQ model to the given training data and parameters using
        gradient ascent.
        ...
        self
        """
        X = np.array([dict2numpy(x)])
        y = np.array([int(y)])

        X, y = self._validate_train_params(X, y, classes)
    ...
    def predict_one(self, x: dict) -> base.typing.ClfTarget:
        """
        Predict class membership index for input sample.

        This function does classification on X.
        ...
        Returns
        -------
        C : Returns predicted value.
        """

        return self.c_w_[np.array([self._costf(dict2numpy(x), p) for p in self.w_]).argmax()]
