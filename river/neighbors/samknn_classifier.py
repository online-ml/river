...
from .base import FunctionWrapper, DistanceFunc
...
class SAMkNNClassifier(base.Classifier):
    ...
    def __init__(self,
        ...
        self.weights: typing.Dict[str, int] = {
            'stm': 0,
            'ltm': 0,
            'cm': 0
        }
        ...
        self.stm = SAMkNNShortTermMemory(n_neighbors=self.n_neighbors, 
                                         dist_func=dist_func,
                                         min_stm_size=self.min_stm_size, 
                                         weighted=self.weighted, 
                                         recalculate_stm_error=recalculate_stm_error)
        self.ltm = SAMkNNLongTermMemory(self.n_neighbors, 
                                        dist_func=dist_func)
    ...
    def learn_one(self, x, y, **kwargs):
        ...
        if optimal_stm_size != self.stm.size():
            ...
            cleaned_new_ltm_items = [new_ltm_item for 
                new_ltm_item in new_ltm_items 
                if all([
                    clean_dist == 0
                    or new_ltm_item[1] != stm_item[1] 
                    or self.ltm.dist_func(new_ltm_item, stm_item) > clean_dist 
                    for stm_item, clean_dist in zip(self.stm, map(self.stm.get_clean_distance, self.stm))])]
    ...
    def predict_proba_one(self, x, memory=None, **kwargs):
        ...
        if memory == 'stm':
            ...
            nearest = sorted(nearest_stm + nearest_ltm, key=operator.itemgetter(1))[:self.n_neighbors]
...
class SAMkNNMemory:
    ...
    def __init__(self,
        ...
        self.items: typing.List[typing.Tuple[dict, base.typing.ClfTarget]] = []
        self.last_search_item: typing.Tuple[dict, base.typing.ClfTarget] = None

    def append(self, 
               item: typing.List[typing.Tuple[dict, base.typing.ClfTarget]] |
                     typing.Tuple[dict, base.typing.ClfTarget]):
    ...
    def search(self, 
               item: typing.Tuple[dict, base.typing.ClfTarget], 
        ...
        search_result = sorted(items_distances, 
                               key=operator.itemgetter(1))[:n_neighbors]
...
class SAMkNNShortTermMemory(SAMkNNMemory):
    ...
    def __init__(self, 
        ...
        self.prediction_histories: typing.List[bool] = {}
    ...
    def get_clean_distance(self, 
        ...
        furthest_distance_same_label = max([item_dist[1] 
                                            for item_dist in nearest 
                                            if item_dist[0][1] == item[1]])
    ...
    def partial_interleaved_test_train_error(self, size: int):
        ...
        if start_idx in self.prediction_histories.keys():
            ...
            item = self.items[-1]
            items_distances = ((p, self.dist_func(item, p)) 
                               for p in self.items[start_idx:-1])
            nearest = sorted(items_distances, 
                             key=operator.itemgetter(1))[:self.n_neighbors]
        ...
        elif start_idx-1 in self.prediction_histories.keys() \
             and not self.recalculate_stm_error:
            ...
            for cur_idx in range(start_idx+1, len(self.items)):
                ...
                items_distances = ((p, self.dist_func(item, p)) 
                                   for p in self.items[start_idx:cur_idx])
                nearest = sorted(items_distances, 
                                 key=operator.itemgetter(1))[:self.n_neighbors]
                probas = collections.defaultdict(lambda: 0)
                for item, dist in nearest:
    ...
    def optimial_size(self):
        ...
        candidate_sizes_scores = {size: self.partial_interleaved_test_train_error(size) 
...
class SAMkNNLongTermMemory(SAMkNNMemory):
    ...
    def compress(self):
        ...
        for cls, cls_count in classes.items():
            ...
            fields, values = zip(*[tuple(zip(*item[0].items())) 
                                   for item in self.items 
                                   if item[1] == cls])

            # Ensure that all items have the same features
            fields = set(fields)
            assert len(fields) == 1, "Not all datapoints have the same fields. Can not compress LTM!"
            fields = fields.pop()

            # Generate and add compressed data
            kmeans = KMeans(n_clusters=max(1, cls_count//2), random_state=0)
            kmeans.fit(values)
            compressed_items += [({cur_field: cur_value for cur_field, cur_value in zip(fields, cur_values)}, cls) 
    ...
    def clean(self, 
        ...
        # Clean items
        self.items = [cur_item for cur_item in self.items 
                                if cur_item[1] != item[1] 
                                or self.dist_func(cur_item, item) > clean_dist]
