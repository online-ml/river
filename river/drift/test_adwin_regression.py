...
def test_basic_drift_at_expected_index():
...
def test_no_drift_constant_stream():
...
def test_no_drift_stable_distribution():
...
def test_immediate_large_shift():
...
def test_estimation_after_drift():
...
def test_width_zero_estimation():
...
def test_max_buckets_variation():
...
def test_adaptive_windowing_basic_update():
...
def test_adaptive_windowing_variance_in_window():
    aw = AdaptiveWindowing()
...
def test_adaptive_windowing_n_detections_increments():
...
def test_adaptive_windowing_sequence_exact_statistics():
...
def test_reset_on_drift_detected():
...
def _seed_adaptive_windowing(aw: AdaptiveWindowing) -> None:
...
def _assert_adaptive_windowing_state_equal(a: AdaptiveWindowing, b: AdaptiveWindowing) -> None:
...
def test_pickle_round_trip(params):
    aw = AdaptiveWindowing(**params)
    _seed_adaptive_windowing(aw)
    revived = pickle.loads(pickle.dumps(aw))
    _assert_adaptive_windowing_state_equal(aw, revived)
...
def test_deepcopy_round_trip():
    ...
    _assert_adaptive_windowing_state_equal(aw, clone)
    for v in [0.5, -1.2, 3.7, 4.1, 2.0]:
