    ...
    *Implementation based on MOA.*
    ...
    def update(self, x: bool):
        ...
        if self.pointer < self.sliding_window_size:
            self.sliding_window[self.pointer] = 1 if (x == 0) else 0
            self.n_one += self.win[self.pointer]
            self.pointer += 1
        else:
            self.n_one -= self.win.pop(0)
            self.win.append(1 if x == 0 else 0)

        if self.pointer == self.sliding_window_size:
            u = self.n_one / self.sliding_window_size
            u_max = u if (self.u_max < u) else u_max
