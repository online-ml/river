...
def test_predict_many_requires_pandas(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(pandas_utils, "import_pandas", _raise_missing_pandas)

    with pytest.raises(ImportError, match="pandas"):
        linear_model.LinearRegression().predict_many(object())  # type: ignore[no-untyped-call]
