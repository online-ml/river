...
class MLPRegressor(base.Regressor, MLP):
    ...
    !!! note
        This model is still very much a work in progress.
    ...
    activations
        The activation functions to use at each layer, including the input and output layers. For
        example, if you specify one hidden layer, then you need to specify three activation
        functions.
    loss
        Loss function. Defaults to `optim.losses.Squared`.
    optimizer
        Optimizer. Default to `optim.SGD`.
    seed
