...
class SAMKNNClassifier(Classifier):
    ...
    def _size_check_stmltm(self):
        ...
        if len(self._stm_labels) + len(self._ltm_labels) > self.max_stm_size + self.max_ltm_size:
            if len(self._ltm_labels) > self.max_ltm_size:
                self._ltm_samples, self._ltm_labels = self._cluster_down(self._ltm_samples,
                                                                         self._ltm_labels)
            else:
                if len(self._stm_labels) + len(self._ltm_labels) > \
                        self.max_stm_size + self.max_ltm_size:
                    stm_shortened = True
                    n_shifts = int(self.max_ltm_size - len(self._ltm_labels) + 1)
                    shift_range = range(n_shifts)
                    self._ltm_samples = np.vstack([self._ltm_samples,
                                                  self._stm_samples[:n_shifts, :]])
                    ...
                    self.stm_distances[:len(self._stm_labels), :len(self._stm_labels)] = \
                        self.stm_distances[n_shifts:len(self._stm_labels) + n_shifts,
    ...
    def _clean_samples(self, samples_cl, labels_cl, only_last=False):
        ...
        if len(self._stm_labels) > self.n_neighbors and samples_cl.shape[0] > 0:
            ...
            for i in loop_range:
                ...
                distances_stm = SAMKNNClassifier._get_distances(self._stm_samples[i, :],
                                                                samples_shortened)
                nn_indices_stm = libNearestNeighbor.nArgMin(self.n_neighbors, distances_stm)[0]
                distances_ltm = SAMKNNClassifier._get_distances(self._stm_samples[i, :],
                                                                samples_cl)
                nn_indices_ltm = libNearestNeighbor.nArgMin(min(len(distances_ltm),
                                                                self.n_neighbors),
                                                            distances_ltm)[0]
                correct_indices_stm = \
                    nn_indices_stm[labels_shortened[nn_indices_stm] == self._stm_labels[i]]
                if len(correct_indices_stm) > 0:
                    dist_threshold = np.max(distances_stm[correct_indices_stm])
                    wrong_indices_ltm = \
                        nn_indices_ltm[labels_cl[nn_indices_ltm] != self._stm_labels[i]]
    ...
    def _learn_one(self, x, y):
        ...
        self._ltm_samples, self._ltm_labels = self._clean_samples(self._ltm_samples,
                                                                  self._ltm_labels,
                                                                  only_last=True)
        ...
        if self.stm_size_adaption is not None:
            ...
            self.stm_distances[len(self._stm_labels) - 1, :len(self._stm_labels) - 1] = \
                distances_stm
            ...
            if new_window_size < old_window_size:
                ...
                self.stm_distances[:len(self._stm_labels), :len(self._stm_labels)] = \
                    self.stm_distances[(old_window_size - new_window_size):
                                       ((old_window_size - new_window_size)
                                        + len(self._stm_labels)),
                                       (old_window_size-new_window_size):
                                       ((old_window_size-new_window_size)
                                        + len(self._stm_labels))]
                ...
                if self.use_ltm:
                    ...
                    old_stm_samples, old_stm_labels = self._clean_samples(old_stm_samples,
                                                                          old_stm_labels)
    ...
    def _learn_one_by_all_memories(self, sample, label, distances_stm):
        ...
        if len(self._stm_labels) == 0:
            ...
            if len(self._stm_labels) < self.n_neighbors:
                predicted_label_stm = self.get_labels_fct(distances_stm, self._stm_labels,
                                                          len(self._stm_labels))[0]
                predicted_label = predicted_label_stm
            else:
                ...
                predicted_label_both = \
                    self.get_labels_fct(np.append(distances_stm, distances_ltm),
                                        np.append(self._stm_labels, self._ltm_labels),
                                        self.n_neighbors)[0]

                if len(self._ltm_labels) >= self.n_neighbors:    # noqa
                    predicted_label_ltm = self.get_labels_fct(distances_ltm, self._ltm_labels,
                                                              self.n_neighbors)[0]
                    ...
                    labels = [predicted_label_stm, predicted_label_ltm, predicted_label_both]
        ...
        self.n_possible_correct_predictions += label in [predicted_label_stm, predicted_label_both,
                                                         predicted_label_ltm]
    ...
    def _predict_by_all_memories(self, sample, label, distances_stm):    # noqa
        ...
        if len(self._stm_labels) == 0:
            ...
            if len(self._stm_labels) < self.n_neighbors:
                ...
                predicted_label_both = \
                    self.get_labels_fct(np.append(distances_new, distances_ltm),
    ...
    def _predict_by_stm(self, sample, label, distances_stm):    # noqa
        ...
        if curr_len > 0:
            predicted_label = self.get_labels_fct(distances_stm, self._stm_labels,
                                                  min(self.n_neighbors, curr_len))[0]
        return predicted_label
...
class STMSizer:
    ...
    def _get_interleaved_test_train_acc(labels, n_neighbours, get_labels_fct, distances_stm):
        ...
        return (STMSizer._acc_score(predLabels[:], labels[n_neighbours:]),
                (predLabels == labels[n_neighbours:]).tolist())

    @staticmethod
    def _get_interleaved_test_train_acc_pred_history(labels, n_neighbours, get_labels_fct,
                                                     prediction_history, distances_stm):
        """
        ...
        return np.sum(prediction_history) / float(len(prediction_history)), prediction_history
    ...
    def _adapt_histories(n_deletions, prediction_histories):
        ...
        for i in range(n_deletions):
            ...
            for j in range(1, len(sortedKeys)):
                prediction_histories[sortedKeys[j] - delta] = \
                    prediction_histories.pop(sortedKeys[j])
        return prediction_histories

    @staticmethod
    def _get_max_acc_window_size(labels, n_neighbours, get_labels_fct,
                                 prediction_histories, distances_stm, min_size=50):
        """
                ...
                if idx in keyset:
                    accuracy, predHistory = \
                        STMSizer._get_interleaved_test_train_acc_pred_history(
                            labels[idx:], n_neighbours, get_labels_fct,
                            prediction_histories[idx], distances_stm[idx:, idx:])
                else:
                    accuracy, predHistory = \
                        STMSizer._get_interleaved_test_train_acc(
    ...
    @staticmethod
    def _get_max_acc_approx_window_size(labels, n_neighbours, get_labels_fct,
                                        prediction_histories, distances_stm, min_size=50):
        """
        Returns the window size with the minimum interleaved
        ...
        if n_samples < 2 * min_size:
            ...
            for numSamplesIt in n_samples_range:
                ...
                if idx in keyset:
                    accuracy, predHistory = \
                        STMSizer._get_interleaved_test_train_acc_pred_history(
                            labels[idx:], n_neighbours, get_labels_fct, prediction_histories[idx],
                ...
                # elif predictionHistories.has_key(idx-1):
                    ...
                    accuracy, predHistory = \
                        STMSizer._get_interleaved_test_train_acc_pred_history(
                            labels[idx:], n_neighbours, get_labels_fct, predHistory,
                            distances_stm[idx:, idx:])
                else:
                    accuracy, predHistory = \
                        STMSizer._get_interleaved_test_train_acc(
