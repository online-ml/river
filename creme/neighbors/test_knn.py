...
def test_regression_predict():
    ...
    model = knn.KNeighborsRegressor(n_neighbors=2, window_size=len(dataset), p=1, weighted=True)

    for x, y in dataset:
        model._nn.update(x, y)

    # dataset[0] and dataset[1] should be the closest
    y_pred = model.predict_one(x={'x': 0, 'y': .4})
    expected = (
        (dataset[0][1] / .4 + dataset[1][1] / .6) /
        (1 / .4 + 1 / .6)
    )
    assert math.isclose(y_pred, expected)

    # dataset[0] is the closest
