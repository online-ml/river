...
class Differ(base.Transformer):
    ...
    Parameters:
        on: The name of the feature for which to compute differences. The type of the values
            must support the `-` operator, for example an `int` or a `datetime.timedelta`.
        when: A function which indicates when an event occurs. This can used to compute
            the difference since the last time an event occured. If `None` then the differences
            will always be computed with regards to the last observation.
        by: Can be used to compute value differences for different groups. If `None` then
            the differences will be computed globally.
        when_name: The name of the of the feature that is returned is automatically built
    ...
    def __init__(self, on: str, when: typing.Callable = None, by: str = None,
                 when_name: str = None):
        self.on = on
        self.by = by
