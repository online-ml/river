...
def test_precisionatk():
    ...
    assert metrics.PrecisionAtK(4)._eval(y_true, y_pred) == 0.25
...
def test_recallatk():
    ...
    assert metrics.RecallAtK(4)._eval(y_true, y_pred) == 0.5
...
def test_f1atk_binary():
    ...
    for _ in range(2000):
        ...
        assert metrics.F1AtK(k=k)._eval(y_true, y_pred) == pytest.approx(
            f1atk_with_subclasses(y_true, y_pred, k)
        )
...
def test_f1atk_relevance():
    ...
    for _ in range(2000):
        ...
        y_pred = random.sample(items_str, pred_len)
        assert metrics.F1AtK(k=k, relevance_threshold=relevance_threshold)._eval(
            y_true_dict, y_pred
        ) == pytest.approx(
            f1atk_with_subclasses(y_true_dict, y_pred, k, relevance_threshold=relevance_threshold)
        )
