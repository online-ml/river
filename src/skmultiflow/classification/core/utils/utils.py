...
def do_naive_bayes_prediction_multilabel(X, observed_class_distribution: dict, attribute_observers: dict,
                                         n_observations):
    if observed_class_distribution == {}:
        # No observed class distributions, all target_values equal
        return {0: 0.0}
    votes = {}
    observed_class_sum = n_observations
    for label_index, observed_class_val in observed_class_distribution.items():
        votes[label_index] = observed_class_val / observed_class_sum
        if attribute_observers:
            for att_idx in range(len(X)):
                if att_idx in attribute_observers:
                    obs = attribute_observers[att_idx]
                    votes[label_index] *= obs.probability_of_attribute_value_given_class(X[att_idx], label_index)
    return votes
