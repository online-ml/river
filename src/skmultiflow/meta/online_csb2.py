...
class OnlineCSB2Classifier(BaseSKMObject, ClassifierMixin, MetaEstimatorMixin):
    ...
    .. code-block:: python
       ...
       online_csb2_classifier = OnlineCSB2Classifier()

       # Train the classifier with the samples provided by the data stream
       while n_samples < max_samples and stream.has_more_samples():
           X, y = stream.next_sample()
           y_pred = online_csb2_classifier.predict(X)
           if y[0] == y_pred[0]:
               correct_cnt += 1
           online_csb2_classifier = online_csb2_classifier.partial_fit(X, y)
           n_samples += 1

       # Display results
       print('{} samples analyzed.'.format(n_samples))
       print('Online CSB2 Classifier performance: {}'.format(correct_cnt / n_samples))
    """
