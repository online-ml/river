...
- Enabled strict typing for `stream.Cache`, along with dedicated tests. Its `directory` argument is annotated as `str | os.PathLike[str]` and normalised to a string on construction, which fixes `repr(cache)` raising a `TypeError` when the cache was built from a `pathlib.Path`. A key containing a dot is also no longer truncated at that dot when an existing cache directory is reopened.
