...
- Added `drift.binary.RDDM`, the Reactive Drift Detection Method. Unlike `DDM`, which latches onto the global minimum error rate and goes deaf after a long stable concept, RDDM re-anchors from a ring buffer of recent error rates, so it reacts several times sooner when a long low-error concept degrades. Ported from the authors' reference implementation in MOA.
